...
<div class="workarea">
    ...
    <?php if ($this->beginCache(
        'arrangementGenreFilter' . CacheHelper::key($dataProvider),
        [
            ...
            <div class="container">
                <div class="filter-block_wrap">
                    <div class="filter-block_select">
                        <?php if ($filterForm->genreCount): ?>
                            <div class="filter-block_select-wrap" style="background: rgb(241, 241, 241);">
                                <span>Выбрано (<?= $filterForm->genreCount ?>)</span>
                        ...
                        <div class="filter-block_select_result">
                            <div class="filter-block_select_result_content nano-select">
                                <div class="nano-content">
                                    <?php
                                    /** @var \store\entities\Genre $genre */
                                    foreach ($filterForm->genreList as $i => $genre): ?>
                                        <div class="filter-block_select_result-item<?= $checked = ($filterForm->genreIsChecked($genre->id) ? ' checked' : '') ?>">
                                            <input type="checkbox" id="genre<?= $i ?>" name="<?= Html::getInputName($filterForm, 'genre[]') ?>" value="<?= $genre->id ?>"<?= $checked ?>>
                                            <label for="genre<?= $i ?>" style="text-transform: capitalize;"><?= $genre->name ?></label>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                    ...
                    <script type="text/javascript">
                        function pjaxReload() {
                            ...
                            $.pjax.reload('#<?= \yii\widgets\Pjax::$autoIdPrefix . \yii\widgets\Pjax::$counter ?>', {
                                history: true,
                                type: 'POST',
                                url: '/arrangement/genre?' + form.serialize()
                            });
                        }
